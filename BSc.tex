...
\begin{abstract}
  ...
  the $W$ bosons, with a secondary novel approach of fitting Delphes $W$
  polarization simulations to data taken from the ATLAS detector. The measured
  helicity fractions from Angular asymmetries are $F_0=0.678 \pm 0.015$,
...
polarizations are illustrated in Figure~\ref{fig:polarization}, where, in this
...
a % er det "the" eller "a" her? hvis det er "the" skal vi nok have kilde på men kan ikke finde
template method was used by generating a different data set for each of the
possible states, which in this case are three polarizations. The same cuts, as
the ones used on the primary data, are applied to the new data sets, which are
then fitted to the primary data in a linear combination. The fitted coefficients
are the contribution of the corresponding state to the primary
data. % måske andet ord end primary data her
If the primary and new data sets are normalised before fitting, the contribution
coefficients will be normalised fractions. % eller bare "also be normalised."
...
Figure~\ref{fig:distributions}. The plot for each of the three polarizations is
...
\begin{figure*}[t!]
  ...
  \begin{subfigure}[t]{0.47\textwidth}
    ...
    \caption{$F_L$ reconstructed distribution with 691 remaining events.}
  \end{subfigure}\hfill{}%
  \begin{subfigure}[t]{0.47\textwidth}
    \centering
    \includegraphics[width=1.2\textwidth]{figures/delphes_ctstarR}
    \caption{$F_R$ reconstructed distribution with 916 remaining events.}
  \end{subfigure}
  \begin{subfigure}[t]{0.5\textwidth}
    \centering
    \includegraphics[width=1.1\textwidth]{figures/delphes_ctstar0}
    \caption{$F_0$ reconstructed distribution with 981 remaining events.}
  \end{subfigure}
  \caption{The figures shows the reconstructed Delphes $\cos \theta^*$ from the
...
of errors is a consequence of statistical errors, due to the small number of event samples
...
Figure~\ref{fig:efficiency}  illustrates that the efficiency is not flat as a function of
$\cos \theta^{*}$, but has lower efficiency in $\cos \theta^{*} < z_+$. This possible
...
Figure~\ref{fig:2dhist}, further reinforces this assertion, where the shift below
