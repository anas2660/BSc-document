...
\begin{figure}[H]
    ...
    pair.}\label{fig:feynmandiagram}%placeholder
\end{figure}
...
helicity fractions.
\begin{figure}[H]
	\includegraphics[width=\linewidth]{figures/angular_dist.png}
...
\end{figure}
%evt fjerner jeg lavels i bunden senere.
...
Spectrometer can be seen in Figure~\ref{detector}
...
\begin{figure}[H]
	...
	\caption{A computer generated image of the ATLAS detector~\cite{detector}}\label{detector}
\end{figure}
...
\subsubsection{Delphes}

\subsection{Results}
%Måske skal den her sektion slettes, eller sættes som en del af data analysis.
...
lepton, and from this we can plot the transverse energy. %Efter jeg lige har
% læst rapporten igennem, så ser det ud til at vi bruger passivt sprog i resten
% af rapporten, vi skal nok lige snakke om hvorvidt vi har tænkt os at bruge
% passivt sprog eller aktivt sprog, vi kan sagtens også bruge passivt sprog til
% analyse delen (siden det er noget som vi har lavet), men det er nok en god ide
% at vi bliver enige og er konsistent, ihvertfald i analysen.
\begin{figure}[H]
  \centering
...
In the ATLAS Open Data we are directly given the missing transverse energy,
which we can then immediately plot if the given event satisfies all the criteria
given in Subsection~\ref{sec:cuts}. It is worth noting that it is given in the
unit MeV, and all we have to do is simply convert to GeV, for a more manageable
unit in the plot.
\begin{figure}[H]
  \centering
...
\begin{figure*}[t!]
    ...
    \caption{The figures shows the reconstructed Delphes $\cos \theta^*$ of the
      ...
      the ones made on the Open ATLAS data.}
\end{figure*}
We can now fit the resulting histograms in a linear combination to the histogram
from the ATLAS data. \cleardoublepage{}
